function aUnfolfddMeta = editorUnfoldString(sAction, sValue)
%function aUnfolfddMeta = editorUnfoldString(sAction, sValue)
%Get\Set Metada string for editorUnfold().
...
% along with dicomMultiFilesEditor.  If not, see <http://www.gnu.org/licenses/>.
    ...
    persistent paUnfolfddMeta; 

    if strcmpi('set', sAction)
       paUnfolfddMeta = sValue;            
    elseif strcmpi('add', sAction)
       paUnfolfddMeta{numel(paUnfolfddMeta)+1} = sValue;
   end   
    
    aUnfolfddMeta = paUnfolfddMeta;
end